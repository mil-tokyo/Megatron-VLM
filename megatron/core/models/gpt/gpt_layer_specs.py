...
# Use this spec to use lower level Transformer Engine modules (required for fp8 training)
def get_gpt_layer_with_transformer_engine_spec_llama(
    num_experts: int = None, moe_grouped_gemm: bool = False, qk_layernorm: bool = False, use_fixed_attn_mask: bool = False
) -> ModuleSpec:
    mlp = _get_mlp_module_spec(
        use_te=True, num_experts=num_experts, moe_grouped_gemm=moe_grouped_gemm
    )
    attn_mask_type = AttnMaskType.causal if not use_fixed_attn_mask else AttnMaskType.arbitrary
    return ModuleSpec(
        module=TransformerLayer,
        submodules=TransformerLayerSubmodules(
            self_attention=ModuleSpec(
...
def get_gpt_layer_with_transformer_engine_spec_clipvit(
    num_experts: int = None, moe_grouped_gemm: bool = False, qk_layernorm: bool = False
) -> ModuleSpec:
    mlp = _get_mlp_module_spec(
        use_te=True, num_experts=num_experts, moe_grouped_gemm=moe_grouped_gemm
