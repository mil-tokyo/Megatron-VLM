...
from megatron.core.transformer.custom_layers.custom_transformer_engine import DotProductAttention
...
class TELinear(te.pytorch.Linear):
    ...
    def __repr__(self):
        # module name + parameters description
        is_bias_enabled = True
        if self.bias is None or self.bias.numel() == 0:
            is_bias_enabled = False
        return f"{self.__class__.__name__}(in_features={self.in_features}, out_features={self.out_features}, bias={is_bias_enabled})"
...
class TEDotProductAttention(te.pytorch.DotProductAttention):
    ...
    def __init__(
        ...
        super().__init__(
            num_attention_heads=self.config.num_attention_heads,  # 16 (CLIP)
            kv_channels=self.config.kv_channels,  # 64 (CLIP)
            attention_dropout=self.config.attention_dropout  # 0.0 (CLIP)
    ...
    def forward(
        ...
        if self.te_forward_mask_type:
            # In CLIP transformer, this way is used in the forward function of the attention layer
