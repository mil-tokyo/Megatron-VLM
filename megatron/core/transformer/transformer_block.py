...
def get_num_layers_to_build(config: TransformerConfig, disable_pipeline_parallelism: bool = False) -> int:
    if disable_pipeline_parallelism:
        # No pipeline parallelism; build all layers.
        print(f"no pipeline parallelism: num_layers={config.num_layers}")
        return config.num_layers

    num_layers_per_pipeline_rank = (
        config.num_layers // parallel_state.get_pipeline_model_parallel_world_size()
    )
    print(f"num_layers_per_pipeline_rank: {num_layers_per_pipeline_rank} = {config.num_layers} // {parallel_state.get_pipeline_model_parallel_world_size()}")
...
def _get_block_submodules(
    ...
    disable_pipeline_parallelism: bool = False
    ...
    # `BaseTransformerLayer` from the `transformer_layer.py` file.
        ...
        if issubclass(spec.module, TransformerBlock):
            ...
            num_layers = get_num_layers_to_build(config, disable_pipeline_parallelism)
...
class TransformerBlock(MegatronModule):
    ...
    def __init__(
        ...
        self.disable_pipeline_parallelism = disable_pipeline_parallelism
        self.submodules = _get_block_submodules(config, spec, disable_pipeline_parallelism)
