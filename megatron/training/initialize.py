...
def initialize_megatron(
    ...
    # print("!skip arguments validation")
    ...
    def finish_mpu_init():
        ...
        try:
            ...
            # print hostname and rank to make it easier to debug
            import socket
            print(f"Failed device initialization; hostname: {socket.gethostname()}, rank: {args.rank}")
            raise e
...
def _initialize_distributed(get_embedding_ranks, get_position_embedding_ranks):
    ...
    print("start _initialize_distributed")
    args = get_args()
    ...
    if torch.distributed.is_initialized():
        ...
        if device_count > 0:
            ...
            print(f"local-rank ({args.local_rank}) vs. device ({device}) = rank ({args.rank}) % device_count ({device_count})")
