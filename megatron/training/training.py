...
def pretrain(
    ...
    model, optimizer, opt_param_scheduler = setup_model_and_optimizer(
        model_provider, model_type, strict_checkpoint_loading=strict_checkpoint_loading
    )
    print(f"model len (pretrain): {len(model)}")
...
def get_model(model_provider_func, model_type=ModelType.encoder_or_decoder, wrap_with_ddp=True):
    ...
    if mpu.get_pipeline_model_parallel_world_size() > 1 and \
       args.virtual_pipeline_model_parallel_size is not None:
        print("Using virtual pipeline model parallelism")
        ...
        print("Using normal model parallelism")
        pre_process = mpu.is_pipeline_first_stage()
        post_process = mpu.is_pipeline_last_stage()
        add_encoder = True
        add_decoder = True
        if model_type == ModelType.encoder_and_decoder:
            print("model_type: encoder_and_decoder")
            ...
            rank = mpu.get_pipeline_model_parallel_rank()
            add_encoder = rank == 0  # Only rank 0 has encoder
            pre_process = rank == 0  # Only rank 0 has pre_process
            model = model_provider_func(
                pre_process=pre_process,
                post_process=post_process,
                add_encoder=add_encoder,
...
def setup_model_and_optimizer(
    ...
    print(f"model len (setup_model_and_optimizer): {len(model)}")
    unwrapped_model = unwrap_model(model)
    print(f"unwrapped_model len (setup_model_and_optimizer): {len(unwrapped_model)}")
    ...
    if args.load is not None or args.pretrained_checkpoint is not None:
        ...
        args.iteration, args.num_floating_point_operations_so_far = load_checkpoint(
            model, optimizer, opt_param_scheduler, strict=strict_checkpoint_loading
        )
...
def training_log(
        loss_dict, total_loss_dict, learning_rate, decoupled_learning_rate, iteration,
    ...
    if args.alert_spike:
        current_loss = [loss_dict[key] for key in loss_dict.keys() if "loss" in key][0].item()

        temp_file.seek(0)
        lines = temp_file.readlines()
        past_loss = float(lines[-1].strip()) if lines else None
        loss_change_rate = (current_loss - past_loss) / past_loss if past_loss else 0

        temp_file.write(f"{current_loss}\n".encode())
        temp_file.flush()

        if loss_change_rate > args.spike_thr:
            message = f"Loss spiked at iteration {iteration}. Loss: {past_loss} -> {current_loss} (thr: {args.spike_thr})"
            if wandb_writer:
                if args.slack_uid is not None:
                    message = f"<@{args.slack_uid}>\n" + message
                wandb_writer.alert(
    ...
    if (wandb_writer or writer) and (iteration % args.tensorboard_log_interval == 0):
        if wandb_writer:
        ...
        if args.alert_spike:
            if writer:
                writer.add_scalar("loss_change_rate", loss_change_rate, iteration)
                writer.add_scalar("loss_change_rate vs samples", loss_change_rate, args.consumed_train_samples)
            if wandb_writer:
                wandb_writer.log({"loss_change_rate": loss_change_rate}, iteration)
    ...
    if iteration % args.log_interval == 0:
        ...
        # compute throughput by our custom method
        ...
        if wandb_writer:
            wandb_writer.log({
                ...
                'tflops': tflops,
                'samples_per_sec_per_replica': samples_per_sec_per_replica,
                'tokens_per_sec': tokens_per_sec,
                'tokens_per_sec_per_replica': tokens_per_sec_per_replica,
                'throughput': throughput,
                'others/skipped_iterations': skipped_iteration
            }, iteration)
        ...
        estimated_time_left = elapsed_time_per_iteration * (
            args.train_iters - iteration)
        estimated_hours_left = estimated_time_left // 3600
        if estimated_hours_left > 0:
            estimated_minutes_left = (estimated_time_left % 3600) // 60
        else:
            estimated_minutes_left = estimated_time_left // 60
        log_string += ' ETA: {:3d}h{:2d}m{:2d}s |'.format(
            int(estimated_hours_left),
            int(estimated_minutes_left),
            int(estimated_time_left % 60))
        log_string += ' consumed samples: {:12d} |'.format(
            args.consumed_train_samples)
        log_string += ' elapsed time per iteration (ms): {:.1f} |'.format(
            elapsed_time_per_iteration * 1000.0)
        log_string += f' throughput per GPU (TFLOP/s/GPU): {tflops:.1f} |'
...
def save_checkpoint_and_time(
    ...
    save_checkpoint(
        ...
        num_floating_point_operations_so_far, checkpointing_context,
        non_persistent_ckpt=False,
        overwrite=True if non_persistent_ckpt else False,
        train_data_iterator=train_data_iterator
    )
    if args.use_distributed_optimizer and args.overlap_param_gather:
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    # Borrowed from https://github.com/llm-jp/Megatron-LM/blob/39135acf57aee423ec6ead34748bdd405793ca38/megatron/training/training.py#L1100
    ...
    while iteration < args.train_iters:
        # Borrowed from https://github.com/llm-jp/Megatron-LM/blob/39135acf57aee423ec6ead34748bdd405793ca38/megatron/training/training.py#L1154
        if (
        ...
        if get_num_microbatches() != num_microbatches and iteration != 0:
            assert get_num_microbatches() > num_microbatches, \
                "number of microbatches should be increasing due to batch size rampup ... %d -> %d." % (
                num_microbatches, get_num_microbatches()
            )
        ...
        report_memory_flag = training_log(
            loss_dict, total_loss_dict, learning_rate, decoupled_learning_rate,
            iteration, loss_scale, report_memory_flag, skipped_iter,
            grad_norm, params_norm, num_zeros_in_grad, skipped_iteration=skipped_iteration
        )
        # StragglerDetector
