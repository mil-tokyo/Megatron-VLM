...
def validate_args(args, defaults={}):
    ...
    if args.decoder_seq_length is not None:
        ...
        f"max_position_embeddings ({args.max_position_embeddings}) should be greater than decoder_seq_length ({args.decoder_seq_length})"
    ...
    # Borrowed from https://github.com/llm-jp/Megatron-LM/blob/39135acf57aee423ec6ead34748bdd405793ca38/megatron/training/arguments.py#L495
...
def core_transformer_config_from_args(args, config_class=None, prefix: str = '') -> TransformerConfig:
    """
    Translate args to core transformer configuration.
    Args:
        args: (argparse.Namespace) arguments
        config_class: class of the configuration
        prefix: (str) prefix added to the arguments; e.g., "language_" or "vision_"

    Returns:
        config: (TransformerConfig) core transformer configuration
    """
    ...
    # extract args with prefix
    if prefix != '':
        args_with_prefix = {k: v for k, v in vars(args_).items() if k.startswith(prefix)}
        # overwrite args with args_with_prefix
        for k, v in args_with_prefix.items():
            args_key_without_prefix = k[len(prefix):]
            setattr(args_, args_key_without_prefix, v)
            # remove args with prefix
            delattr(args_, k)

    # Translate args to core transformer configuration
    kw_args = {}
    for f in dataclasses.fields(config_class):
        if hasattr(args_, f.name):
            kw_args[f.name] = getattr(args_, f.name)
    kw_args['persist_layer_norm'] = not args_.no_persist_layer_norm
    kw_args['layernorm_zero_centered_gamma'] = args_.apply_layernorm_1p
    kw_args['layernorm_epsilon'] = args_.norm_epsilon
    kw_args['deallocate_pipeline_outputs'] = True
    kw_args['pipeline_dtype'] = args_.params_dtype
    kw_args['batch_p2p_comm'] = not args_.overlap_p2p_comm
    kw_args['num_moe_experts'] = args_.num_experts
    kw_args['rotary_interleaved'] = args_.rotary_interleaved
    if args_.swiglu:
        kw_args['activation_func'] = F.silu
        kw_args['gated_linear_unit'] = True
        kw_args['bias_activation_fusion'] = args_.bias_swiglu_fusion
    else:
        kw_args['bias_activation_fusion'] = args_.bias_gelu_fusion
    if args_.squared_relu:
        assert not args_.swiglu
        kw_args['activation_func'] = squared_relu
    if args_.init_method_xavier_uniform:
        kw_args['init_method'] = torch.nn.init.xavier_uniform_
        kw_args['scaled_init_method'] = torch.nn.init.xavier_uniform_
    if args_.group_query_attention:
        kw_args['num_query_groups'] = args_.num_query_groups
    else:
        kw_args['num_query_groups'] = None
    kw_args['config_logger_dir'] = args_.config_logger_dir
...
def _add_logging_args(parser):
    ...
    group.add_argument('--wandb-tags', type=str, default='',
                          help='Comma separated list of tags for wandb; e.g., "tag1,tag2"')
    group.add_argument("--alert-spike", action="store_true",
                       help="Alert loss spike")
    group.add_argument("--spike-thr", type=float, default=2,
                       help="Loss spike threshold")
    group.add_argument("--slack-uid", type=str, default=None,
                       help="Slack user ID")
...
def _add_multimodal_args(parser):
    ...
    group.add_argument('--freeze-adapter', action='store_true', default=False)
    group.add_argument('--image-token-id', type=int, default=-1)
    group.add_argument('--pad-token-id', type=int, default=-1)
    group.add_argument('--config-json-path', type=str, default=None)
    group.add_argument('--command', type=str, default=None)
    group.add_argument('--dataloader-save', type=str, default=None, help="energon dataloader state save path")
    group.add_argument('--slurm-job-id', type=int, default=None, help="slurm job id")
    group.add_argument('--slurm-output-name', type=str, default=None, help="slurm job name")
    group.add_argument('--add-class-token', action='store_true', default=False)
    group.add_argument('--training-stage', type=int, default=0, help="training stage; 1 or 2")
    group.add_argument(
        '--use-fixed-attn-mask', action='store_true', default=False,
        help="use fixed attention mask or not; WIP"
    )
    group.add_argument('--env-params', type=str, default=None, help="environment parameters")
    group.add_argument('--multimodal-train-samples', type=int, default=None, help="number of training samples")
    group.add_argument('--model-subtype', type=str, default=None, help="model subtype")
    return parser
