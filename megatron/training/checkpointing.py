...
def get_checkpoint_name(checkpoints_path, iteration, release=False,
    ...
    print(f"world size: {mpu.get_tensor_model_parallel_world_size()}")
    ...
    if not pipeline_parallel:
        ...
        print(f"common_path (not pipeline): {common_path}")
    else:
        # if pipeline parallel is enabled, then this block has to be executed
        # Note: distributed init: False, use_dist_ckpt: False
        common_path = os.path.join(checkpoints_path, directory,
                f'mp_rank_{tensor_rank:02d}_{pipeline_rank:03d}')
        print(f"common_path (pipeline): {common_path}")

    if expert_parallel:
        common_path = common_path + f'_{expert_rank:03d}'
        print(f"common_path (expert): {common_path}")
...
def find_checkpoint_rank_0(checkpoints_path, iteration, release=False):
    ...
    if os.path.isfile(filename):
        print(f"Found checkpoint (no pipeline, no expert): {filename}")
        return filename
    ...
    if os.path.isfile(filename):
        print(f"Found checkpoint (no pipeline, expert): {filename}")
        return filename
    ...
    filename = get_checkpoint_name(checkpoints_path, iteration, release,
                                   ...
                                   expert_parallel=False, expert_rank=0)
    if os.path.isfile(filename):
        print(f"Found checkpoint (pipeline, no expert): {filename}")
        return filename
    ...
    filename = get_checkpoint_name(checkpoints_path, iteration, release,
                                   ...
                                   expert_parallel=True, expert_rank=0)
    if os.path.isfile(filename):
        print(f"Found checkpoint (pipeline, expert): {filename}")
        return filename
    ...
    if dist_checkpointing.check_is_distributed_checkpoint(filename):
        print(f"Found distributed checkpoint: {filename}")
        return filename
...
def save_checkpoint(
        ...
        checkpointing_context=None, pipeline_rank=None, expert_rank=None, tensor_rank=None, pipeline_parallel=None,
        expert_parallel=None, non_persistent_ckpt=False,
        train_data_iterator=None, overwrite=False
):
    ...
    if overwrite:
        ...
        save_name_iter = iteration
    checkpoint_name = get_checkpoint_name(
        save_dir, save_name_iter, release=False, pipeline_parallel=pipeline_parallel,
    ...
    if overwrite and os.path.exists(checkpoint_dir):
        # remove the checkpoint if it already exists
        print_rank_0(f"Removing existing checkpoint: {checkpoint_dir}")
        shutil.rmtree(checkpoint_dir, ignore_errors=True)  # TODO: change this to thread-safe removal
        # wait for all ranks to finish removing the checkpoint
        if torch.distributed.is_initialized():
    ...
    print(f"use_dist_ckpt: {args.use_dist_ckpt}")
    ...
    save_dataloader_state(train_data_iterator, save_name_iter, getattr(args, "dataloader_save", None))
    start_misc = time()
...
def save_dataloader_state(train_iterator, iteration, dataloader_save_path):
    ...
    print(f"train_iterator: {train_iterator}, dataloader_save_path: {dataloader_save_path}, iteration: {iteration}")
    ...
    if not hasattr(train_iterator, "save_state"):
        print(f"train_iterator ({train_iterator}) does not have save_state method. Skipping dataloader checkpoint save.")
        return
    ...
    # if not first_rank:
    #     print(f"Skipping dataloader checkpoint save for rank {mpu.get_data_parallel_rank()}.")
    #     return
    ...
    try:
        ...
        print(f"Error saving dataloader state on rank {dp_rank} on host {hostname}: {e}")
        raise e
...
def generate_state_dict(args, model, optimizer, opt_param_scheduler,
    ...
    print(f"generate state dict | size of model: {len(model)} | use_dist_ckpt: {use_dist_ckpt}")
    if len(model) == 1:
        print(f"type of model to save: {type(model[0])}")
...
def load_args_from_checkpoint(args, load_arg='load',
    ...
    _set_arg('add_position_embedding', force=False)
...
def load_checkpoint(model, optimizer, opt_param_scheduler, load_arg='load', strict=True):
    ...
    # Optimizer.
    print_rank_0(f"Optimizer loading config; release: {release}, finetune: {args.finetune}, no_load_optim: {args.no_load_optim}")
    if not release and not args.finetune and not args.no_load_optim:
        print_rank_0("Trying to load optimizer from checkpoint ...")
        try:
            # Load state dict.
            if optimizer is not None:
                optimizer.load_state_dict(state_dict['optimizer'])
                print_rank_0("  successfully loaded optimizer from checkpoint!!")

            # Load distributed optimizer's custom parameter state.
            # For distributed checkpoint it's already loaded in load_state_dict above
            if args.use_distributed_optimizer and not is_dist_ckpt:
                print_rank_0("Trying to load distributed optimizer's custom parameter state from checkpoint ...")
                ...
                print_rank_0("  successfully loaded distributed optimizer's custom parameter state from checkpoint!!")
