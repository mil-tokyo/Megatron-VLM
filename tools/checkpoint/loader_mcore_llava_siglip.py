...
def load_llava_args_from_checkpoint(args):
    print("loader_mcore_llava_siglip | Loading LLAVA args from checkpoint ... (loader_mcore_llava_siglip.py")
...
def _load_checkpoint(queue, args):
    ...
    try:
        ...
        print("loader_mcore_llava_siglip | Unable to import Megatron, please specify the path to Megatron using --megatron-path. Exiting.")
        queue.put("exit")
        exit(1)
    ...
    def check_for_arg(arg_name, default=None):
        if getattr(margs, arg_name, None) is None:
            if default is not None:
                ...
                print(f"loader_mcore_llava_siglip | Checkpoint does not specify the argument {arg_name}. Exiting.")
                print(f"loader_mcore_llava_siglip | Arguments: {margs}")
                queue.put("exit")
                exit(1)
    ...
    if args.true_vocab_size is not None:
        ...
        if args.true_vocab_size is not None and true_vocab_size != args.true_vocab_size:
            print("loader_mcore_llava_siglip | Both --true-vocab-size and --vocab-file specified and the vocab size does not match, aborting.")
            queue.put("exit")
            exit(1)
    else:
    ...
    print("loader_mcore_llava_siglip | Sending metadata...")
    queue.put(md)
    print("loader_mcore_llava_siglip | Metadata sent.")

    def queue_put(name, msg):
        print(f"loader_mcore_llava_siglip | sending {name}")
        msg["name"] = name
        queue.put(msg, block=True, timeout=None)
        # if name.startswith("vision transformer layer"):
    ...
    print(f"loader_mcore_llava_siglip | vision_transformer_layer_num: {vision_transformer_layer_num}")
    ...
    for layer_num in range(vision_transformer_layer_num):
        ...
        for tp_rank, model in enumerate(models):
            ...
            dense_bias.append(layer.self_attention.linear_proj.bias.data)
        ...
        message["dense bias"] = torch.cat(dense_bias, dim=0)
