...
print("saver_llava_hf | Launching saver_llava_hf.py")
...
def save_checkpoint(queue: mp.Queue, args):
    print("saver_llava_hf | start save_checkpoint")
    ...
    def queue_get(name=None):
        ...
        if val == "exit":
            print("saver_llava_hf | Loader exited, exiting saver")
            exit(1)
        if name is not None and args.checking and val["name"] != name:
            val_name = val["name"]
            print(f'saver_llava_hf | Unexpected message. Expecting "{name}" but got "{val_name}". Exiting saver.')
            exit(1)
        if name is not None:
            print(f"saver_llava_hf | received {name}")
    ...
    def check_message(msg):
        ...
        if len(msg.keys()) > 0:
            print(f"saver_llava_hf | Unexpected values in {msg_name}:")
            for key in msg.keys():
                print(f"   {key}")
            print("saver_llava_hf | Exiting. If you want to ignore this, use the argument --no-checking.")
            exit(1)
    ...
    print("saver_llava_hf | got checkpoint args")
    ...
    for i_layer in range(vision_layer_num):
        ...
        try:
            ...
            print(f"saver_llava_hf | Since we are not going to use other vision model, we hardcoded the shape of qkv weight")
            print(f"saver_llava_hf | If you encounter this error, please check the shape of qkv weight you're trying to load")
            raise e

        # set bias
        qkv_bias = message["qkv bias"]

        qkv_bias = qkv_bias.view(16, -1)
        q_bias, k_bias, v_bias = torch.split(qkv_bias, qkv_bias.size(-1) // 3, dim=1)
    ...
    for i_layer in range(text_conf.num_hidden_layers):
        ...
        kv_channels = text_conf.hidden_size // text_conf.num_attention_heads
        num_groups = text_conf.num_key_value_heads
        q_input_dim = kv_channels * text_conf.num_attention_heads //num_groups
        k_input_dim = kv_channels
        v_input_dim = kv_channels

        qkv_params_size = math.prod(qkv_weight.size())
        qkv_last_dim = qkv_params_size // (text_conf.num_attention_heads * (q_input_dim + k_input_dim + v_input_dim))
        qkv_weight = qkv_weight.view(text_conf.num_attention_heads, -1, qkv_last_dim)

        qkv_weight = torch.split(qkv_weight, [q_input_dim, k_input_dim, v_input_dim], dim=1)
    ...
    with suspend_nn_inits():
        print("\n\nsaver_llava_hf | Saving model to disk ...")
        ...
        print("\n\nsaver_llava_hf | Comparing state_dict:")
        ...
        print(f"\nsaver_llava_hf | Unexpected keys: {sorted(list(unexpected_keys))}")
        print(f"\nsaver_llava_hf | Missing keys: {sorted(list(missing_keys))}")
    ...
    print(f"saver_llava_hf | model saved to {args.save_dir}")
